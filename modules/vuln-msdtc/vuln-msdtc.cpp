...
/**
 * The Constructor
 * creates a new MSDTCVuln Module, 
 * MSDTCVuln is an example for binding a socket & setting up the Dialogue & DialogueFactory
 * 
 * 
 ...
 */
MSDTCVuln::MSDTCVuln(Nepenthes *nepenthes)
{
	...
	m_DialogueFactoryDescription = "creates dialogues to emulate msdtc";
...
bool MSDTCVuln::Init()
{
	...
	try
	{
		sList = *m_Config->getValStringList("vuln-msdtc.ports");
 ...
 * 
 * creates a new MSDTCVulnDialogue
 * 
 ...
 */
Dialogue *MSDTCVuln::createDialogue(Socket *socket)
{
	return new MSDTCDialogue(socket);
}
...
extern "C" int32_t module_init(int32_t version, Module **module, Nepenthes *nepenthes)
{
	if (version == MODULE_IFACE_VERSION) {
        *module = new MSDTCVuln(nepenthes);
        return 1;
    } else {
        return 0;
    }
