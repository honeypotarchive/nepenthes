...
/**
 * The Constructor
 * creates a new PNPVuln Module, 
 * PNPVuln is an example for binding a socket & setting up the Dialogue & DialogueFactory
 * 
 * 
 ...
 */
PNPVuln::PNPVuln(Nepenthes *nepenthes)
{
	...
	m_DialogueFactoryDescription = "creates dialogues to emulate MS05-039";
...
bool PNPVuln::Init()
{
	...
	try
	{
		sList = *m_Config->getValStringList("vuln-pnp.ports");
 ...
 */
Dialogue *PNPVuln::createDialogue(Socket *socket)
{
	return new PNPDialogue(socket);
}
