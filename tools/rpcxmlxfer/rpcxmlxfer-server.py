...
__rcsid__ = "$Id: rpcxmlxfer-server.py 1914 2005-08-26 11:08:01Z md $"
...
def init_session(user, sysid, timestamp, metadata):
  ...
  return (md5.new(str(time.time())+sysid+user).hexdigest(), "Ok - Server $Rev: 1914 $ welcomes you!")
...
def test():
  ...
  print init_session('userid', 'sensorid', xmlrpclib.DateTime(time.time()),
                     {'clientsoftware': 'rpcxmlxfer-test $Rev: 1914 $'})
...
parser = OptionParser(usage=usage, version='$Rev: 1914 $')
...
if options.verbose > 2:
  print "rpcxmlxfer-client $Rev: 1914 $ started."
  print options
